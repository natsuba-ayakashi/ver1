...
def parse_css_colors(soup):
    """
    旧フォーマットの <style> から .pX クラスの文字色を抽出
    例: .p12 { color:#FF0000; }
    """
    css_colors = {}
    for style in soup.find_all("style"):
        matches = re.findall(r"\.p(\d+)\s*{\s*color\s*:\s*(#[0-9A-Fa-f]{6})", style.text)
        for pid, color in matches:
            css_colors[f"p{pid}"] = color
    return css_colors
...
def analyze_old(content, config):
    ...
    css_colors = parse_css_colors(soup)

    for p in soup.find_all("p", class_="player"):
        ...
        # 名前抽出
        name_tag = p.find("b")
        name = name_tag.get_text(strip=True) if name_tag else "不明キャラ"
